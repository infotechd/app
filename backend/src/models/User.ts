...
UserSchema.methods.comparePassword = async function(candidatePassword: string): Promise<boolean> {
  ...
  if (!this.senha) {
    // Lança um erro ou retorna falso se a senha não foi carregada (select: false)
    console.error("Erro em comparePassword: Campo 'senha' não foi selecionado na query para este usuário.");
    // Lançar um erro pode ser mais apropriado para sinalizar um problema de programação
    // throw new Error("Campo 'senha' não carregado para comparação.");
    return false;
  }
